...
function Chicken() {
  ...
  const eyeRingt = new THREE.Mesh(
    new THREE.BoxBufferGeometry(5 * zoom, 5 * zoom, 15 * zoom),
    new THREE.MeshPhongMaterial({ color: 0xf0619a, flatShading: true })
  );
  eyeRingt.position.z = 50 * zoom;
  eyeRingt.position.x = -4 * zoom;
  eyeRingt.position.y = -15 * zoom;

  eyeRingt.castShadow = true;
  eyeRingt.receiveShadow = true;
  chicken.add(eyeRingt);
