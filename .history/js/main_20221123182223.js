...
function Chicken() {
  ...
  earRight.position.z = 50 * zoom;
  earRight.position.x = 4 * zoom;
  earRight.position.y = -15 * zoom;
  ...
  earLeft.position.z = 50 * zoom;
  earLeft.position.x = -4 * zoom;
  earLeft.position.y = -15 * zoom;
  ...
  const eye = new THREE.Mesh(
    new THREE.BoxBufferGeometry(1 * zoom, 3 * zoom, 5 * zoom),
    new THREE.MeshPhongMaterial({ color: 0x000000, flatShading: true })
  );
  eye.position.z = 46 * zoom;
  eye.position.x = 10 * zoom;
  eye.position.y = -12 * zoom;
  ...
  const rear = new THREE.Mesh(
    new THREE.BoxBufferGeometry(5 * zoom, 4 * zoom, 5 * zoom),
    new THREE.MeshPhongMaterial({ color: 0xf0619a, flatShading: true })
  );
  rear.position.z = 25 * zoom;
  rear.position.x = 1 * zoom;
  rear.position.y = -20 * zoom;
  ...
  const nose = new THREE.Mesh(
    new THREE.BoxBufferGeometry(3 * zoom, 4 * zoom, 5 * zoom),
    new THREE.MeshPhongMaterial({ color: 0xdd0c0c, flatShading: true })
  );
  nose.position.z = 45 * zoom;
  nose.position.x = 1 * zoom;
  nose.position.y = 0 * zoom;
...
function animate(timestamp) {
  ...
  if (lanes[currentLane].type === "car" || lanes[currentLane].type === "truck") {
    let chickenMinX = chicken.position.x - (chickenSize * zoom) / 2;
    let chickenMaxX = chicken.position.x + (chickenSize * zoom) / 2;
    let vechicleLength = { car: 60, truck: 105 }[lanes[currentLane].type];
    lanes[currentLane].vechicles.forEach((vechicle) => {
      let carMinX = vechicle.position.x - (vechicleLength * zoom) / 2;
      let carMaxX = vechicle.position.x + (vechicleLength * zoom) / 2;
      if (chickenMaxX > carMinX && chickenMinX < carMaxX && die == false) {
